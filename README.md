...
- Binary frost vs non‑frost thin‑section image classification  
- Multiclass microstructure image classification

using CNN backbones (MobileNetV2, EfficientNetV2B0, ResNet50), Bayesian neural networks (BNNs), and Score‑CAM interpretability. An interactive demo is available on Hugging Face Spaces, and a dedicated script also evaluates **full photomicrographs** end‑to‑end, generating Score‑CAM overlays, BNN uncertainty, and publication‑ready metrics and figures on held‑out images.
...
- Strong data augmentation and class‑balanced losses/weights.  
- Two‑phase training: K‑fold cross‑validation + final training on the full train set.  
...
## Score‑CAM and Full‑Image Analysis
...
A dedicated **full photomicrograph pipeline** resizes complete thin‑section images, runs all five architectures (CNN, BNN, MobileNetV2, EfficientNetV2B0, ResNet50), computes BNN Monte Carlo uncertainty, and produces Score‑CAM overlays, confusion matrices, metric comparison plots, and CSV summaries for direct use in figures and quantitative analysis.
...
- `full_photomicrographs_*.py` – Full‑image evaluation with Score‑CAM overlays, BNN uncertainty, and cross‑model metric comparison on held‑out photomicrographs  
...
The dataset and the results are publicly available at:  
[https://doi.org/10.17605/OSF.IO/TYDSM](https://doi.org/10.17605/OSF.IO/TYDSM)
...
[skouki@ualg.pt](mailto:skouki@ualg.pt)
